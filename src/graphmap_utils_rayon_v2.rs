...
where V: NodeTrait + Send + Sync{
    ...
    neighborhoods.par_iter().for_each(|entry| {
        let &key = entry.key();
        let vec: &Vec<V> = entry.value();        //need to create variable?
    ...
    N: NodeTrait + Eq + Send + Sync + Debug,
{
    ...
    neighborhoods.par_iter().for_each(|entry| {
        ...
        // base
        h_mutex.lock().unwrap().add_edge(*u, v_min, ());
        for node in neighbors {
            //eprintln!("[h] adding: {:?} -> {:?}", node, v_min);
            h_mutex.lock().unwrap().add_edge(*node, v_min, ());
        }
...
pub fn prune_os<N: NodeTrait + Send + Sync + Debug>(
    h: DiGraphMap<N, ()>,
    tree: DiGraphMap<N, ()>,
